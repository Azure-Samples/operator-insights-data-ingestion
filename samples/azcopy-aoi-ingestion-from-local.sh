...
echo "Uploading files from local path $sourceDataFilePath to $ingestionUrl/$dataType/testdata/date-$(date +%d-%m-%Y)"
azcopy copy "$sourceDataFilePath" "$ingestionUrl/$dataType/testdata/date-$(date +%d-%m-%Y)?$sasToken" --recursive=true
